...
def main():
    ...
    while True:
        ...
        if cv2.waitKey(1) & 0xFF == ord('a'): #Start or Play countinue
            if game_over:
                continue
        ...
        if cv2.waitKey(1) & 0xFF == ord('r'): # Reset game
            game_over = False
            score = 0

        if cv2.waitKey(1) & 0xFF == ord('q'): # Exit game
            break
        ...
        if game_over:
            ...
            if game_start:
                ...
                soundplay = 0
                if score == 0:
                    cv2.putText(frame, f'PRESS A TO PLAY', (700, height-100), cv2.FONT_HERSHEY_SIMPLEX, 2, (random.randint(1,255), random.randint(1,255), random.randint(1,255)), 8)
                    cv2.putText(frame, f'PRESS Q TO Exit', (width-600, 100), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 8)
                else:
                    status = 0
                    cv2.putText(frame, f'Score : {score}', (700, 300), cv2.FONT_HERSHEY_SIMPLEX, 4, (0, 0, 255), 8)
